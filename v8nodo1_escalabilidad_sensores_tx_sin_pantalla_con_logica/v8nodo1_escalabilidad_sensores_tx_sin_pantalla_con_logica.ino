...
void enviarDatosLoRa(int temps[], int hums[], bool errores[], bool atemp[], bool ahum[], bool gas[], int zonas) {
  ...
  // Generar mensaje neutro dinámicamente
  ...
  if (mensaje != mensajeNeutro) {
    // Si es diferente, activa los relés (conmuta)
    ...
    for (int i = 0; i < NUM_RELES; i++) {
      digitalWrite(RELE_PINS[i], HIGH);
    }
    Serial.println("Mensaje neutro detectado. Relés no se activan.");
  }
}
