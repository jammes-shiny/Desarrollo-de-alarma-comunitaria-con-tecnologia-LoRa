...
// Sensor MQ135 digital
#define MQ135_PIN 4
#define UMBRAL_GAS 1  // Cambia a 0 si tu sensor devuelve LOW cuando hay gas
...
//crea nodo
  connectWiFi();  // Enciende y conecta WiFi
  if (WiFi.status() == WL_CONNECTED) {
    client.setServer(mqtt_server, 1883);
    reconnectMQTT();
    ...
    payload += String(NODO) + ", ";       // nodo
    payload += String(0) + ", ";          // temperatura
    payload += String(0) + ", ";          // humedad
    payload += String(0) + ", ";          // gas
    payload += String(0);                 // error

    // Publicar por MQTT
    client.publish(topic.c_str(), payload.c_str());

    delay(1000); // Espera para asegurar el envío
    disconnectWiFi();  // Apaga WiFi
  } else {
...
void loop() {
  ...
  int gas = !digitalRead(MQ135_PIN); // Lectura digital del sensor MQ135
  ...
  bool current_gas = (gas == UMBRAL_GAS); // true si hay gas

  if (current_error != last_error || current_atemp != last_atemp || current_ahum != last_ahum || current_gas != last_gas) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      payload = String(id) + ", ";             // id
      payload += String(current_error ? 0 : (current_atemp ? 1 : 0)) + ", "; //temperatura
      payload += String(current_error ? 0 : (current_ahum ? 1 : 0)) + ", "; //humedad
      payload += String(current_gas ? 1 : 0) + ", "; //gas
      payload += String(current_error ? 1 : 0); //temp err
  ...
  display.print("Gas: ");
  display.println(current_gas ? "Detectado" : "Normal");
  display.display();
