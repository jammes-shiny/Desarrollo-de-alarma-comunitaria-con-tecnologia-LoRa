...
void loop() {
  ...
  if (current_error != last_error || current_atemp != last_atemp || current_ahum != last_ahum || current_gas != last_gas) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      payload = "{\"domicilio\": \"" + String(NODO) + "\", ";
      payload += "\"alerta temperatura\": " + String(current_error ? 0 : (current_atemp ? 1 : 0)) + ", ";
      payload += "\"alerta humedad\": " + String(current_error ? 0 : (current_ahum ? 1 : 0)) + ", ";
      payload += "\"alerta gas\": " + String(current_gas ? 1 : 0) + ", ";
      payload += "\"alerta temp error\": " + String(current_error ? 1 : 0) + "}";
